...
CBR_RATE = 11250000 # 90mbps #4375000  # 35 Mbps = 4,375,000 B/s
N_ROUNDS = 3        # 测试轮数
...
def parse_itg_logs(node):
    ...
    try:
        ...
        print(f"  ✓ 最终解析结果: 吞吐量={throughput_mbps:.2f}Mbps, 延迟={avg_delay_ms:.1f}ms, 丢包={packet_loss_percent:.1f}%")
        return result
...
def run_ditg_mptcp_test(net):
    """运行D-ITG MPTCP测试"""
    print("\n" + "="*60)
    print("D-ITG MPTCP性能测试开始")
    print(f"目标速率: {CBR_RATE*8/1000000:.1f} Mbps (35 Mbps)")  # 修复显示
    print("="*60)
    ...
    with open(CSV_LOG, 'w', newline='') as csvfile:
        ...
        for scheduler in SCHEDULERS:
            ...
            for round_id in range(1, N_ROUNDS + 1):
                ...
                # 启动发送端
                print(f"启动发送端 (sta1) -> h1:10.0.1.10 (CBR {CBR_RATE*8/1000000:.1f} Mbps, {TEST_DURATION}s)...")
                test_duration_ms = TEST_DURATION * 1000
                send_result = sta1.cmd(f'ITGSend -a 10.0.1.10 -T TCP -C {CBR_RATE} '
    ...
    with open(CSV_SUMMARY, 'w', newline='') as summary_file:
        ...
        summary_writer.writerow([
            ...
            'Avg_Jitter_ms', 'Avg_Packet_Loss_%', 'Completed_Rounds'
        ])
        
        print(f"{'调度器':<12} {'平均吞吐(Mbps)':<15} {'平均延迟(ms)':<12} {'平均抖动(ms)':<12} {'丢包率(%)':<10} {'完成轮数':<8}")
        print("-" * 80)
        ...
        for scheduler in SCHEDULERS:
            ...
            if results:
                ...
                summary_writer.writerow([
                    scheduler, avg_throughput, avg_delay, avg_jitter, avg_loss, len(results)
                ])
                
                print(f"{scheduler:<12} {avg_throughput:>13.2f}   {avg_delay:>10.1f}    {avg_jitter:>10.3f}    {avg_loss:>8.1f}   {len(results):>6}")
            else:
    ...
    if all_results:
        ...
        if valid_results:
            ...
            # 计算效率
            target_mbps = CBR_RATE * 8 / 1000000
            print(f"📊 目标速率: {target_mbps:.1f} Mbps")
            print("📊 各调度器效率:")
            for scheduler, results in valid_results.items():
                avg_thr = sum(r['throughput_mbps'] for r in results) / len(results)
                efficiency = (avg_thr / target_mbps) * 100
                print(f"   {scheduler}: {avg_thr:.2f} Mbps ({efficiency:.1f}% 效率)")
